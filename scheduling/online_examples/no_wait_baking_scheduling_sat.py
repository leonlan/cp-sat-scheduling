...
def solve_with_cp_sat(recipes, resources, orders):
        ...
        for batch in range(order.quantity):
            ...
            for task in recipe.tasks:
                ...
                if previous_end is None:
                    start = model.new_int_var(start_work, horizon,
                                            f'start{suffix}')
                ...
                size = model.new_int_var(task.min_duration, task.max_duration,
                                       f'size{suffix}')
                end = None
                if task == recipe.tasks[-1]:
                    # The order must end after the due_date. Ideally, exactly at the
                    # due_date.
                    tardiness = model.new_int_var(0, horizon - due_date,
                                                f'end{suffix}')
    ...
    for resource in resources:
        ...
        if resource.capacity == 1:
            ...
            model.add_cumulative(intervals, [1] * len(intervals),
                                resource.capacity)
