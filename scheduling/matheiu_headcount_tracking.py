...
def cumulative_with_start_time(model: cp_model.CpModel, variables: Variables, tasks: Set[str]):
    ...
    for task in tasks:
        ...
        for t in time_range:
            # Capture start time
            model.add(variables.var_task_starts[task] == t).only_enforce_if(variables.var_task_starts_presence[task, t])
...
if __name__ == '__main__':
    ...
    # variables.var_task_starts = {task: model.new_int_varFromDomain(domain=cp_model.Domain.from_values(),
    ...
    model.add_no_overlap(variables.var_intervals.values())
